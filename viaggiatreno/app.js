...
let LIMITE_SCROLL_TAB;

let indiceTraduzione;

let infoPopupName;

let lastSearchedItems = new Array();
let lastSearchedItemsTreni = new Array();
let lastSearchedItemsStazioni = new Array();
let lastSearchedItemsOrigine = new Array();
let lastSearchedItemsDestinazione = new Array();
...
VT.App = function () {
    ...
    $('#bottone-cerca').click(function (event) {
        ...
        if ($(this).hasClass('on')) {
            ...
            if ($cercaTreno.data('ricorda') && lastSearchedItems.length > 0) {
                let valore = lastSearchedItems[lastSearchedItems.length - 1];
    ...
    $('#infomobility-popup a.chiudi').click(function (event) {
        $('#infomobility-popup').fadeOut()
        $('#infomobility-popup #rssInfomobilita').empty()
        return false;
    });

    $('#smartcaring-popup a.chiudi').click(function (event) {
        $('#smartcaring-popup').fadeOut()
        $('#smartcaring-popup #rssInfomobilita').empty()
        return false;
    });

    $('#info-mobilita a.chiudi').click(function (event) {
        $('info-mobilita').fadeOut()
        $('#info-mobilita #accordionGenericInfomob').empty()
        return false;
    });

    $('#mod-prog a.chiudi').click(function (event) {
        $('mod-prog').fadeOut()
        $('#mod-prog #accordionGenericInfomob').empty()
        return false;
    });
    ...
    $('.input-localita-impostaViaggio').autocomplete({
        source: function (request, response) {
            ...
            let suggerimenti = [];
            let risp = [];
            let baseUrl = "./resteasy/viaggiatreno/autocompletaStazioneImpostaViaggio/";
            let testo = "";
            ...
            if ((testo.length < 2) || (request.term.click && testo.length == 0)) {
                ...
                $.ajax({
                    ...
                    dataType: 'html',
                    success: function (data, textStatus, xhr) {
                        let risposte = data.split("\n");
                        let suggerimenti = new Array(risposte.length - 1);
                        let dati = {};
    ...
    $('.input-localita').autocomplete({
        source: function (request, response) {
            ...
            let suggerimenti = [];
            let risp = [];
            let baseUrl = "./resteasy/viaggiatreno/autocompletaStazione/";
            let testo = "";
    ...
    $('#form-impostaviaggio').submit(function (event) {
        ...
        let partenza = $partenza.data('dati')[$partenza.val()];
        let arrivo = $arrivo.data('dati')[$arrivo.val()];
        //######################################## Web storage HTML5
    ...
    function download(strData, strFileName, strMimeType) {
        ...
        let D = document, A = arguments, a = D.createElement("a"), d = A[0], n = A[1], t = A[2] || "application/pdf";
        let newdata = "data:" + strMimeType + ";base64," + escape(strData);
        a.href = newdata;
    ...
    let linguaSelezionata = $.cookie('LANG');
    ...
    let $selettoreRegione = $('#selettore-regione');
    for (let name in regioniData) {
    ...
    $('#cartina-regioni a').on('click', function (event) {
        if ($(this).parent().is(':visible')) { // Check necessario per bug IE8
            let idRegione = $(this).data('regione');
    ...
    let redirectRegione = getParameterByName("idreg");
    if (redirectRegione !== '') {
        for (let codiceRegione in regioniData) {
...
VT.App.prototype.chiudiTab = function (index) {
    ...
    let attivo = $("#tabs .tab").eq(index).is(".attivo");
...
VT.App.prototype.getTemplate = function (templateName) {
    let templateAccessor = templateName + "Template";
...
VT.App.prototype.renderTemplate = function (selector, templateName, context) {
    ...
    let renderedTemplate = template(context);
...
function updatejScrollPane() {
    if (!$.browser.msie) {
        $.each($('.scrollable'), function (index, val) {
            let $scrol = $(this);
            let jspData = $scrol.closest('.jspScrollable').jScrollPane().data();
...
function manageWebPartenze(webPartenze) {
    let len = webPartenze.length;
    let partenze = new Array(len);
   
            
    for (let i = 0; i < len; i++) {
        let webPartenza = webPartenze[i];
        let nonPartitoEstero = true;
        if (webPartenza.oraPartenzaEstera != null && !webPartenza.circolante){
            let partenzaEstero = webPartenza.oraPartenzaEstera;
            let convertPartenzaEstero = formatTime(partenzaEstero);
            let dataAttuale = formatTime(Date.now());
        ...
        partenze[i] = {
            ...
            imgRitardo: webPartenza.compImgRitardo2,
...
function manageWebArrivi(webArrivi) {
    let len = webArrivi.length;
    let arrivi = new Array(len);   
    
     
    
    for (let i = 0; i < len; i++) {
        let webArrivo = webArrivi[i];
        let nonPartitoEstero = true;
        if (webArrivo.oraPartenzaEstera != null && !webArrivo.circolante){
            let partenzaEstero = webArrivo.oraPartenzaEstera;
            let convertPartenzaEstero = formatTime(partenzaEstero);
            let dataAttuale = formatTime(Date.now());
            if(convertPartenzaEstero < dataAttuale){
            	nonPartitoEstero = false;
        ...
        arrivi[i] = {
            ...
            imgRitardo: webArrivo.compImgRitardo2,
            nonPartitoEstero : nonPartitoEstero
        };
    }
    return arrivi;
...
function manageDettaglioTreno(treno, tabIndex) {
    let primaFermata = treno.fermate[0];
    
    let ultimaFermata = null;
    for (let i = treno.fermate.length - 1; i >= 0; i--) {
        if (treno.fermate[i].actualFermataType !== 3) {
            ultimaFermata = treno.fermate[i];
            break;
        }
    }
    if (ultimaFermata == null) 
        ultimaFermata = treno.fermate[treno.fermate.length - 1];

    let orarioPartenzaOriginale = treno.compOrarioPartenzaZero;
    ...
    let orarioPartenza = treno.compOrarioPartenza;
    let orarioPartenzaCambiato = orarioPartenza !== null && orarioPartenzaOriginale !== null && orarioPartenzaOriginale != orarioPartenza;
    let orarioPartenzaEffettivo = formatTime(primaFermata.effettiva);

    let orarioArrivoOriginale = treno.compOrarioArrivoZero;
    let orarioArrivo = treno.compOrarioArrivo;
    let orarioArrivoCambiato = orarioArrivo !== null && orarioArrivoOriginale !== null && orarioArrivoOriginale != orarioArrivo;
    let orarioArrivoEffettivo = (!!ultimaFermata.arrivoReale) ? formatTime(ultimaFermata.effettiva) : treno.compOrarioArrivoZeroEffettivo;

    let orarioUltimoRilevamento = formatTime(treno.oraUltimoRilevamento);

    let cambiNumero = treno.cambiNumero;
    for (let i = 0; i < cambiNumero.length; i++) {
        cambiNumero[i].numero = treno.compNumeroTreno;
    }
    let nProvv = treno.cambiNumero.length + (treno.subTitle.length > 0 ? 1 : 0);

    let descrizioneVCO = treno.descrizioneVCO;
    ...
    let alertRitardo = false;
    if (ultimaFermata.arrivoReale === null && orarioUltimoRilevamento !== "--:--" && treno.arrivato === false) {
        let now = moment().tz("Europe/Rome");

        //Creo la data di ultimo rilevamento
        let dataUltimoRilevamentoString = now.get("year") + "-" + (now.get("month") + 1) + "-" + (now.get("date")) + " " + orarioUltimoRilevamento;
        let dataUltimoRilevamento = moment(dataUltimoRilevamentoString, "YYYY-MM-DD HH:mm").tz("Europe/Rome");

        //Aggiungo 15 minuti alla data di ultimo rilevamento
        let dataCompare = dataUltimoRilevamento.add(15, "minutes");

        //Differenza in secondi tra la data di adesso e la data di ultimo rilevamento + 15 minuti
        let diffSecond = Math.floor((moment.duration(now.diff(dataCompare))).asSeconds());
    ...
    let strDataPartenzaTreno = moment(treno.dataPartenzaTreno).format('DD/MM/YYYY');
    
    let partenzaEstero = treno.oraPartenzaEstera;
    let nonPartitoEstero = true;
    ...
    // Verifico se l'ultimo rilevamento è più vecchio di 15 minuti
    let arrivato = !!(ultimaFermata.arrivoReale) || treno.arrivato;
    let tsNow = new Date(Date.now() - (15 * 60 * 1000));
    if (treno.oraUltimoRilevamento  == null && tsNow > primaFermata.programmata) {
        orarioPartenzaEffettivo = "--:--";
        orarioArrivoEffettivo = "--:--";
    } else if (treno.oraUltimoRilevamento  != null && tsNow > treno.oraUltimoRilevamento && !arrivato) {
        orarioArrivoEffettivo = "--:--";
    }
    
    if(alertRitardo)
    	treno.compRitardoAndamento[indiceTraduzione] = "";
    	
    if(treno.tipoTreno == "ST"){
		treno.compRitardoAndamento[indiceTraduzione] = "non partito";
		treno.compImgRitardo = "/vt_static/img/legenda/icone_legenda/nonpartito.png";
		treno.stazioneUltimoRilevamento = "";
    ...
    return {
        ...
        destinazione: treno.destinazioneEstera ? treno.destinazioneEstera : ultimaFermata.stazione,
        ...
        origine: treno.origineEstera ? treno.origineEstera : primaFermata.stazione,
...
function manageTratta(webTratta) {
    let len = webTratta.treni.length;
    let treni = new Array(len);
    for (let i = 0; i < len; i++) {
...
function manageInfoFermata(stazione, ritardo, tsUltimoRilevamento, oraPartenzaPrimaFermata) {
    let prima = stazione.first;
    let ultima = stazione.last;
    let partenzaProg;
    let partenzaEff;
    let orarioPartEff;
    let orarioArrEff;
    ...
    let orarioEff = formatTime(prima ? stazione.fermata.partenzaReale : stazione.fermata.arrivoReale);
    ...
    if(!(orarioArrEff)) {
        if ((tsUltimoRilevamento == null && tsNow > oraPartenzaPrimaFermata) 
            || (tsUltimoRilevamento != null && tsNow > tsUltimoRilevamento)) {
            orarioEff = "--:--";
        }
    }
    if(!(orarioPartEff)) {
    ...
    return {
        ...
        binarioProgrammato: stazione.fermata.binarioProgrammatoPartenzaDescrizione != null ? stazione.fermata.binarioProgrammatoPartenzaDescrizione : stazione.fermata.binarioProgrammatoArrivoDescrizione,
        binarioEffettivo: stazione.fermata.binarioEffettivoPartenzaDescrizione != null ? stazione.fermata.binarioEffettivoPartenzaDescrizione : stazione.fermata.binarioEffettivoArrivoDescrizione,
...
VT.DettaglioStazione.prototype.aggiorna = function () {
    let stazione = this.stazione;
    let tabIndex = this.tabIndex;
    let orario = this.orarioRicerca.toDate();
    let dettaglioStazione = this;

    app.mostraLoader();
    ViaggiaTrenoService.getStazioniCitta({
        stazione: stazione,
        "$callback": function (a, b, webLocalita) {
            let nomeStazione = null;
            if (webLocalita && webLocalita.length > 0) {
                for (let i = 0; i < webLocalita.length; i++) {
                ...
                "$callback": function (a, b, codiceRegioneStazioneSelected) {
                    ...
                    ViaggiaTrenoService.getDettaglioStazione({
                        ...
                        "$callback": function (a, b, webDettaglioStazione) {
                            let nome = $('#contenuto-' + tabIndex + " option:selected").text();
                            ...
                            //####################################################################################
                            let $bottoneStazione = $('#contenuto-' + tabIndex + " .link-mappa-citta").unbind('click');
...
VT.DettaglioTratta = function (tratta, tabIndex) {
    this.tabIndex = tabIndex;
    let filtriAV = [];
    for (let i = 0; i < app.map.filtroTreniCorrente.length; i++) {
...
VT.DettaglioTreno = function (treno, origine, datapartenza, tabIndex, webAndamentoTreno) {
    ...
    this.tabIndex = tabIndex;
    let dettaglioTreno = manageDettaglioTreno(webAndamentoTreno, tabIndex);
    ...
    $('#news-section-' + tabIndex).hide();

    
        let infomobilityButton = $('<button>', {
            class: 'news-button',
            text: 'Infomobilità'
        });
        
        let smartCaringButton = $('<button>', {
            class: 'news-button',
            text: 'Smart Caring'
        });
        infomobilityButton.hide();
        smartCaringButton.hide();
        $('#news-section-' + tabIndex).append(infomobilityButton);
        $('#news-section-' + tabIndex).append(smartCaringButton);
        
        NewsService.getInfomobilityNews({
            correlationId: getRandomUUID(),
            trainNumber: treno,

            "$callback" : function(status, _b, value) {
                if (status < 300 && status > 199) {
                    infomobilityButton.click(value, function(event) {
                        let response = event.data;
                        let ulElement = $('<ul>', {
                            id: 'accordionGenericInfomob'
                        });
                        for (let i = 0; i < response.length; i++) {
                            let liElement = $('<li>', {
                                class: 'editModeCollapsibleElement'
                            });
                            
                            // Creazione dell'elemento <a> con gli attributi e l'evento onclick
                            let anchorElement = $('<a>', {
                                href: '#',
                                id: 'headingNewsAccordionFeed'+i,
                                class: 'headingNewsAccordion',
                                text: value[i].title,
                                click: function() {
                                    infoCollapse(this.id);
                                }
                            });
                    
                            if (response[i].evidenzia) {
                                anchorElement.addClass('inEvidenza');
                            }
                            // Creazione dell'elemento <div> con la classe e l'attributo style
                            let divElement = $('<div>', {
                                class: 'boxAcc',
                                style: 'display: block;'
                            ...
                            // Aggiunta dell'elemento <a> e del div all'elemento <li>
                        ...
                        $('#infomobility-popup #rssInfomobilita').append(ulElement);
                        $('#infomobility-popup').fadeIn()
                    });        
                    if (value.length > 0) {
                        infomobilityButton.show();
                        $('#news-section-' + tabIndex).show();
                    }
                }
            }
        });

        let partenzaDate = new Date(parseInt(datapartenza, 10));
        let searchDate = moment(partenzaDate).tz("Europe/Rome").format("YYYY-MM-DD");

	if (webAndamentoTreno.codiceCliente != 63 && 
		webAndamentoTreno.codiceCliente != 64 && 
		!webAndamentoTreno.compNumeroTreno.trim().startsWith("FR") && 
		!webAndamentoTreno.compNumeroTreno.trim().startsWith("FB") && 
		!webAndamentoTreno.compNumeroTreno.trim().startsWith("FA") && 
		!webAndamentoTreno.compNumeroTreno.trim().startsWith("IC")) {
        NewsService.getSmartCaringNews({
            correlationId: getRandomUUID() ,
            commercialTrainNumber: treno,
            searchDate: searchDate,
            originCode: origine,
            "$callback" : function(status, _b, value) {
                if (status < 300 && status > 199) {
                    smartCaringButton.click(value, function(event) {
                        let response = event.data;
                        let ulElement = $('<ul>', {
                            id: 'accordionGenericInfomob'
                        });
                        for (let i = 0; i < response.length; i++) {
                            let infoNote = response[i].infoNote;
                            let liElement = $('<li>', {
                                class: 'editModeCollapsibleElement'
                            });
                            ...
                            let anchorElement = $('<a>', {
                                href: '#',
                                id: 'headingNewsAccordionSmart' + i,
                                class: 'headingNewsAccordion inEvidenza',
                                text: formatDateTime(response[i].insertTimestamp),
                                click: function() {
                                    infoCollapse(this.id);
                                }
                            ...
                            // Creazione dei paragrafi all'interno del div con i testi
                            let paragraphs = [infoNote];
                            ...
                            // Aggiunta dei paragrafi al div
                            divElement.append(paragraphElements);
                        ...
                        $('#smartcaring-popup #rssInfomobilita').append(ulElement);
                        $('#smartcaring-popup').fadeIn()
                    });
                    if (value.length > 0) {
                        $('#news-section-' + tabIndex).show();
                        smartCaringButton.show();
                    } else {
    ...
    let contenuto = $('div.contenuto.treno:visible');
...
VT.DettaglioTreno.prototype.aggiornaFermata = function (stazione, orario, ritardo, tsUltimoRilevamento, oraPartenzaPrimaFermata) {
    app.renderTemplate("#contenuto-" + this.tabIndex + " .contenitore-info-fermata", "info-fermata", manageInfoFermata(stazione, ritardo, tsUltimoRilevamento, oraPartenzaPrimaFermata));
    this.stazione = stazione;
...
VT.DettaglioTreno.prototype.aggiorna = function (webAndamentoTreno) {
    ...
    let dettaglioTreno = manageDettaglioTreno(webAndamentoTreno);
        ...
        "$callback": function (a, b, webTratteCanvas) {
            if (webTratteCanvas) {
                ...
                canvas = new VTCanvas(
                        ...
                        useTouchEvents,
                        function (stazione) {
                            let primaFermata = webAndamentoTreno.fermate[0];
                            for (let i = 0; i < webAndamentoTreno.fermate.length; i++) {
                                if (webAndamentoTreno.fermate[i].id == stazione.trattaCanvas.id) {
                                    orarioAndamento = getOrario(webAndamentoTreno.fermate[i]);
                                }
                            }
                            dettaglioTreno.aggiornaFermata(stazione.trattaCanvas, 
                                                            orarioAndamento, 
                                                            webAndamentoTreno.ritardo, 
                                                            webAndamentoTreno.oraUltimoRilevamento, 
                                                            primaFermata.programmata);
                        }
                );
...
VT.MappaCitta = function (stazione, nomeStazione, regione, tabIndex) {
    ...
    ViaggiaTrenoService.getDettaglioStazione({
        ...
        "$callback": function (a, b, webDettaglioStazione) {
            ...
            let mappaCitta = new ubiest.maps.Map($("#contenuto-" + tabIndex + " .mappa-citta")[0], opzioniMappa);
            ...
            let marker = new ubiest.maps.Marker({
                position: centro,
                map: mappaCitta,
                icon: pinPoint
            });
            ...
            let infowindow = new InfoBox(opzioniFumetto);
            ...
            $(infowindow.div_).find('.pulsante-partenze').live('click', function (event) {
                let codiceStazione = $(this).data('stazione');
...
function manageProgrammaOrario(soluzioni, shouldOpenTrainDetail) {
    ...
    return $.map(soluzioni, function (soluzione, index) {
        let numeroCambi = soluzione.vehicles.length - 1;

        let primoTreno = soluzione.vehicles[0];
        let ultimoTreno = soluzione.vehicles[numeroCambi];

        soluzione.vehicles[numeroCambi].ultimo = true;

        let categoriaSenzaAndamenti = false;
        for (let i = 0; i < soluzione.vehicles.length; i++) {
...
VT.ProgrammaOrario.prototype.aggiorna = function () {
    let tabIndex = this.tabIndex;
    let origine = this.origine;
    let destinazione = this.destinazione;
    let codLocOrig = parseInt(origine.replace(/^(S?)[0]*/, ""), 10);
    let codLocDest = parseInt(destinazione.replace(/^(S?)[0]*/, ""), 10);

    if (this.ultimaDataCercata === null || !this.data.isSame(this.ultimaDataCercata, 'day')) {
        app.mostraLoader();
        let that = this;

        let url = "./resteasy/viaggiatreno/soluzioniViaggioNew/" + codLocOrig + "/" + codLocDest + "/" + that.data.format("YYYY-MM-DDTHH:mm:ss").toString();
...
VT.ProgrammaOrario.prototype.render = function () {
    let shouldOpenTrainDetail = this.data.isSame(moment(), 'day');
    let dataDa = this.data.clone();
    let dataA = dataDa.clone();
    let fascia = this.fascia;
    switch (this.fascia) {
        case 0:
            dataDa.startOf('day');
            dataA.hours(6);
    ...
    let tutteSoluzioni = manageProgrammaOrario(this.webSoluzioniViaggioNew.soluzioni, shouldOpenTrainDetail);
    let soluzioniFiltrate = $.grep(tutteSoluzioni, function (e, i) {
        let ret = false;
        let dataACopied = dataA.clone(); // le copio perche altrimenti mi cambia la Label del range
        let dataDaCopied = dataDa.clone();  // le copio perche altrimenti mi cambia la Label del range
...
VT.DettaglioSoluzione = function (soluzione, shouldOpenTrainDetail, tabIndex) {
    ...
    let categoriaSenzaAndamenti = false;
    for (let i = 0; i < soluzione.cambi.length; i++) {
    ...
    $('#contenuto-' + tabIndex + " .attivo tr, tr.cliccabile.attivo").click(function (event) {
        let numeroTreno = $(this).data('treno');
        $.ajax({
            ...
            success: function (data, textStatus, xhr) {
                ...
                {
                    let origine = data.split("\n")[0].split("|")[1].split("-")[1];
                    let datapartenza = data.split("\n")[0].split("|")[1].split("-")[2];
...
VT.App.prototype.apriDettaglioStazione = function (stazione, descrStaz) {
    let tabIndex = this.apriTab('stazione');
    if (tabIndex != -1) {
        let dettaglio = new VT.DettaglioStazione(stazione, descrStaz, tabIndex);
        this.tabs.push(dettaglio);
    }
};

VT.App.prototype.apriDettaglioTratta = function (tratta) {
    let tabIndex = this.apriTab('tratta');
    if (tabIndex != -1) {
        let dettaglio = new VT.DettaglioTratta(tratta, tabIndex);
        this.tabs.push(dettaglio);
    }
};
...
VT.App.prototype.apriDettaglioTrattaInfoMob = function (tratta) {
    let tabIndex = this.apriTab('tratta');
    if (tabIndex != -1) {
        let dettaglio = new VT.DettaglioTrattaInfoMob(tratta, tabIndex);
        this.tabs.push(dettaglio);
    }
};
//////////////////////////////////////////
...
VT.App.prototype.apriDettaglioTreno = function (treno, origine, datapartenza) {
    ...
    ViaggiaTrenoService.getAndamentoTreno({
        ...
        dataPartenza: datapartenza,
        "$callback": function (a, b, webAndamentoTreno) {
                let tabIndex = this.apriTab('treno');
                if (tabIndex != -1) {
                    let dettaglio = new VT.DettaglioTreno(treno, origine, datapartenza, tabIndex, webAndamentoTreno);
                    this.tabs.push(dettaglio);
                }
                app.nascondiLoader();
        }.bind(this)
...
VT.App.prototype.apriMappaCitta = function (stazione, nomeStazione, regione) {
    let tabIndex = this.apriTab('mappa');
    if (tabIndex != -1) {
        let dettaglio = new VT.MappaCitta(stazione, nomeStazione, regione, tabIndex);
        this.tabs.push(dettaglio);
    }
};

VT.App.prototype.apriProgrammaOrario = function (data, fascia, nomeOrigine, origine, nomeDestinazione, destinazione) {
    let tabIndex = this.apriTab('programma-orario');
    if (tabIndex != -1) {
        let dettaglio = new VT.ProgrammaOrario(data, fascia, nomeOrigine, origine, nomeDestinazione, destinazione, tabIndex);
        this.tabs.push(dettaglio);
    }
};

VT.App.prototype.apriDettaglioSoluzione = function (soluzione, shouldOpenTrainDetail) {
    let tabIndex = this.apriTab('dettaglio-soluzione');
    if (tabIndex != -1) {
        let dettaglio = new VT.DettaglioSoluzione(soluzione, shouldOpenTrainDetail, tabIndex);
        this.tabs.push(dettaglio);
    }
};
...
function apriDettaglioSoluzione(numeroTreno) {
    let codiceOrigine = "";
    let datapartenza = "";
    let targetUrl = "./resteasy/viaggiatreno/cercaNumeroTreno/" + numeroTreno;
    $.ajax({
        'url': targetUrl,
        'type': 'GET',
        'dataType': 'json',
...
function stampaPdf(contenitore)
{
    ...
    let contenuto = $(contenitore);

    let numTreno = contenuto.find('#st-input-numero-treno').val();
    $partenza = contenuto.find('#st-input-localita-arrivo');
    let partVal = $partenza.val();
    let partDati = $partenza.data('dati');
    let partCod = "";
    if (partDati !== undefined)
    {
        partCod = partDati[partVal];
    ...
    let dataVal = contenuto.find('.inputDate').val();
    let data = moment(dataVal, "DD/MM/YYYY").tz("Europe/Rome");
    let dataString = data.format('DD/MM/YYYY');
...
function recuperaDatiStampaArrivo(numeroTreno, stazione, stazioneDesc, data) {
    let dpartenza = data.replace(/\//g, "-");
    let targetUrl = getBaseURL() + "StampaTreno";
    let comunicazione;
    let creaPdf;
    let provvedimenti;
    ...
    $.ajax({
        ...
        'success': function (data) {
            ...
            if (creaPdf) {
                let url = getBaseURL() + "certificato/certificato.jsp" + "?comunicazione=" + escape(comunicazione) + "&provvedimenti=" + escape(provvedimenti);
                if (isSafari()) {
