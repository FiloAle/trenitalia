...
L.Util = {
	...
	bind: function (fn, obj) { // (Function, Object) -> Function
		let args = arguments.length > 2 ? Array.prototype.slice.call(arguments, 2) : null;
		return function () {
...
L.Mixin.Events = {
	...
	fireEvent: function (type, data) { // (String[, Object])
		...
		let event = L.Util.extend({}, data, {
			type: type,
			target: this
		});

		let listeners, i, len, eventsObj, contextId;
...
(function () {
	...
	let touch = !window.L_NO_TOUCH && (function () {

		let startName = 'ontouchstart';
...
L.DomUtil = {
	...
	getScaleString: function (scale, origin) {
		...
		let preTranslateStr = L.DomUtil.getTranslateString(origin.add(origin.multiplyBy(-1 * scale))),
...
L.LatLng.prototype = {
	...
	distanceTo: function (other) { // (LatLng) -> Number
		...
		let R = 6378137, // earth radius in meters
...
L.Map = L.Class.extend({
	...
	setMaxBounds: function (bounds) {
		...
		let minZoom = this.getBoundsZoom(bounds, true);
	...
	invalidateSize: function (animate) {
		...
		let offset = oldSize._subtract(this.getSize())._divideBy(2)._round();
	...
	getMaxZoom: function () {
		let z1 = this.options.maxZoom === undefined ? Infinity : this.options.maxZoom,
	...
	getPixelBounds: function () {
		let topLeftPoint = this._getTopLeftPoint();
	...
	layerPointToLatLng: function (point) { // (Point)
		let projectedPoint = L.point(point).add(this.getPixelOrigin());
		return this.unproject(projectedPoint);
	},

	latLngToLayerPoint: function (latlng) { // (LatLng)
		let projectedPoint = this.project(L.latLng(latlng))._round();
	...
	containerPointToLatLng: function (point) {
		let layerPoint = this.containerPointToLayerPoint(L.point(point));
	...
	_initContainer: function (id) {
		let container = this._container = L.DomUtil.get(id);
	...
	_initPanes: function () {
		...
		let zoomHide = ' leaflet-zoom-hide';
	...
	_updateZoomLevels: function () {
		...
		for (i in this._zoomBoundLayers) {
			if (this._zoomBoundLayers.hasOwnProperty(i)) {
				let layer = this._zoomBoundLayers[i];
	...
	_initEvents: function (onOff) {
		...
		let events = ['dblclick', 'mousedown', 'mouseup', 'mouseenter',
	...
	_fireMouseEvent: function (e) {
		...
		let containerPoint = this.mouseEventToContainerPoint(e),
	...
	_latLngToNewLayerPoint: function (latlng, newZoom, newCenter) {
		let topLeft = this._getNewTopLeftPoint(newCenter, newZoom).add(this._getMapPanePos());
...
L.Projection.Mercator = {
	...
	project: function (latlng) { // (LatLng) -> Point
		...
		let ts = Math.tan(0.5 * ((Math.PI * 0.5) - y)) / con;
		y = -r2 * Math.log(ts);
...
L.TileLayer = L.Class.extend({
	...
	bringToFront: function () {
		let pane = this._map._panes.tilePane;
	...
	_tileShouldBeLoaded: function (tilePoint) {
		...
		if (!this.options.continuousWorld) {
			let limit = this._getWrapTileNum();
	...
	_getTilePos: function (tilePoint) {
		let origin = this._map.getPixelOrigin(),
		    tileSize = this.options.tileSize;
	...
	_getSubdomain: function (tilePoint) {
		let index = (tilePoint.x + tilePoint.y) % this.options.subdomains.length;
		return this.options.subdomains[index];
	},

	_createTileProto: function () {
		let img = this._tileImg = L.DomUtil.create('img', 'leaflet-tile');
	...
	_getTile: function () {
		if (this.options.reuseTiles && this._unusedTiles.length > 0) {
			let tile = this._unusedTiles.pop();
	...
	_createTile: function () {
		let tile = this._tileImg.cloneNode(false);
	...
	_tileOnError: function () {
		...
		let newUrl = layer.options.errorTileUrl;
		if (newUrl) {
			this.src = newUrl;
		}
...
L.TileLayer.WMS = L.TileLayer.extend({
	...
	onAdd: function (map) {
		...
		let projectionKey = parseFloat(this.wmsParams.version) >= 1.3 ? 'crs' : 'srs';
...
L.TileLayer.Canvas = L.TileLayer.extend({
	...
	_createTileProto: function () {
		let proto = this._canvasProto = L.DomUtil.create('canvas', 'leaflet-tile');
		proto.width = proto.height = this.options.tileSize;
	},

	_createTile: function () {
		let tile = this._canvasProto.cloneNode(false);
		tile.onselectstart = tile.onmousemove = L.Util.falseFn;
		return tile;
	},
...
L.Icon.Default = L.Icon.extend({
	...
	_getIconUrl: function (name) {
		...
		let path = L.Icon.Default.imagePath;
...
L.Marker = L.Class.extend({
	...
	_animateZoom: function (opt) {
		let pos = this._map._latLngToNewLayerPoint(this._latlng, opt.zoom, opt.center);
	...
	_onMouseClick: function (e) {
		let wasDragged = this.dragging && this.dragging.moved();
...
L.Popup = L.Class.extend({
	...
	onAdd: function (map) {
		...
		let animFade = map.options.fadeAnimation;
	...
	_adjustPan: function () {
		...
		if (containerPos.x + containerWidth  + padding.x > size.x) {
...
L.Marker.include({
	...
	bindPopup: function (content, options) {
		let anchor = L.point(this.options.icon.options.popupAnchor) || new L.Point(0, 0);
...
L.Map.include({
	...
	_animatePathZoom: function (e) {
		let scale = this.getZoomScale(e.zoom),
...
L.Path = L.Browser.svg || !L.Browser.vml ? L.Path : L.Path.extend({
	...
	_initPath: function () {
		let container = this._container = this._createElement('shape');
...
L.Map.include(L.Browser.svg || !L.Browser.vml ? {} : {
	_initPathRoot: function () {
		...
		let root = this._pathRoot = document.createElement('div');
...
L.Path = (L.Path.SVG && !window.L_PREFER_CANVAS) || !L.Browser.canvas ? L.Path : L.Path.extend({
	...
	_drawPath: function () {
		let i, j, len, len2, point, drawMethod;
...
L.LineUtil = {
	...
	clipSegment: function (a, b, bounds, useLastCode) {
		let codeA = useLastCode ? this._lastCode : this._getBitCode(a, bounds),
...
L.Polyline = L.Path.extend({
	...
	spliceLatLngs: function () { // (Number index, Number howMany)
		let removed = [].splice.apply(this._latlngs, arguments);
	...
	closestLayerPoint: function (p) {
		let minDistance = Infinity, parts = this._parts, p1, p2, minPoint = null;
	...
	_convertLatLngs: function (latlngs, overwrite) {
		let i, len, target = overwrite ? latlngs : [];
	...
	_clipPoints: function () {
		let points = this._originalPoints,
		    len = points.length,
		    i, k, segment;
...
L.Polygon = L.Polyline.extend({
	...
	_clipPoints: function () {
		...
		for (let i = 0, len = this._parts.length; i < len; i++) {
			let clipped = L.PolyUtil.clipPolygon(this._parts[i], this._map._pathViewport);
			if (clipped.length) {
	...
	_getPathPartStr: function (points) {
		let str = L.Polyline.prototype._getPathPartStr.call(this, points);
...
L.GeoJSON = L.FeatureGroup.extend({
	...
	addData: function (geojson) {
		...
		let layer = L.GeoJSON.geometryToLayer(geojson, options.pointToLayer);
...
L.extend(L.GeoJSON, {
	...
	coordsToLatLng: function (coords, reverse) { // (Array, Boolean) -> LatLng
		let lat = parseFloat(coords[reverse ? 0 : 1]),
...
L.DomEvent = {
	...
	disableClickPropagation: function (el) {
		...
		for (let i = L.Draggable.START.length - 1; i >= 0; i--) {
	...
	_filterClick: function (e, handler) {
		let elapsed = L.DomEvent._lastClick && (e.timeStamp - L.DomEvent._lastClick);
	...
	_onDown: function (e) {
		...
		if (e.touches && e.touches.length === 1 && L.Browser.touch && this._longPress) {
			this._longPressTimeout = setTimeout(L.bind(function () {
				let dist = (this._newPos && this._newPos.distanceTo(this._startPos)) || 0;
...
L.Map.Drag = L.Handler.extend({
	...
	_onViewReset: function () {
		...
		let pxCenter = this._map.getSize()._divideBy(2),
...
L.Map.ScrollWheelZoom = L.Handler.extend({
	...
	_onWheelScroll: function (e) {
		...
		let left = Math.max(40 - (+new Date() - this._startTime), 0);
...
L.extend(L.DomEvent, {
	...
	addDoubleTapListener: function (obj, handler, id) {
		...
		function onTouchEnd(e) {
			/*jshint forin:false */
			if (L.Browser.msTouch) {
				let idx = trackedTouches.indexOf(e.pointerId);
				if (idx === -1) {
					return;
				}
		...
		let endElement = L.Browser.msTouch ? document.documentElement : obj;
...
L.Map.TouchZoom = L.Handler.extend({
	...
	_onTouchStart: function (e) {
		...
		let p1 = map.mouseEventToLayerPoint(e.touches[0]),
	...
	_onTouchEnd: function () {
		...
		let origin = this._getScaleOrigin(),
		    center = map.layerPointToLatLng(origin),
	...
	_getScaleOrigin: function () {
		let centerOffset = this._centerOffset.subtract(this._delta).divideBy(this._scale);
...
L.Map.BoxZoom = L.Handler.extend({
	...
	_onMouseMove: function (e) {
		let startPoint = this._startLayerPoint,
		    box = this._box,
...
L.Map.include({
	...
	panBy: function (offset, duration, easeLinearity, noMoveStart) {
		...
		let newPos = this._getMapPanePos().subtract(offset);
	...
	_animatePanIfClose: function (center) {
		...
		let offset = this._getCenterOffset(center)._floor();
...
L.TileLayer.include({
	_animateZoom: function (e) {
		...
		let scaleStr = L.DomUtil.getScaleString(e.scale, e.origin),
	...
	_getLoadedTilesPercentage: function (container) {
		let tiles = container.getElementsByTagName('img'),
		    i, len, count = 0;
	...
	_stopLoadingImages: function (container) {
		let tiles = Array.prototype.slice.call(container.getElementsByTagName('img')),
		    i, len, tile;
...
L.Map.include({
	...
	locate: function (/*Object*/ options) {
		...
		let onResponse = L.bind(this._handleGeolocationResponse, this),
	...
	_handleGeolocationResponse: function (pos) {
		...
		if (options.setView) {
			let zoom = Math.min(this.getBoundsZoom(bounds), options.maxZoom);
