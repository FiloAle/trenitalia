...
let sc_code_ver="H.24.1_020412"

let s_account="fdsviaggiatrenoprod"
let s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,asf,swf"
s.linkInternalFilters="javascript:,viaggiatreno.com,viaggiatreno.it,viaggiatreno.eu"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"
s.dstStart="03/25/"+new Date().getFullYear();
...
function s_doPlugins(s) {
  ...
  /* Determine bounce rate for all visits */
	...
	s.clickPast(s.firstPage,'event7','event8');
  ...
  if(s.pageName&&!s.eVar31) s.eVar31="D=pageName";
  if(s.channel&&!s.eVar32) s.eVar32="D=ch";
  
  /* Hierarchies */
  if(!s.hier1&&s.channel&&s.prop8) s.hier1=s.prop8+','+s.channel; {
	if(s.hier1) 
		for(let i=1;i<8;i++) {
  ...
  s.prop11=s.getTimeParting('h','+1',currDate.getFullYear()) // Set hour
  if(s.prop11&&!s.eVar11) s.eVar11="D=c11";  
  s.prop12=s.getTimeParting('d','+1',currDate.getFullYear()) // Set day
  if(s.prop12&&!s.eVar12) s.eVar12="D=c12"; 
  s.prop13=s.getTimeParting('w','+1',currDate.getFullYear()) // Set Weekend Weekday
  if(s.prop13&&!s.eVar13) s.eVar13="D=c13";

  /* Get new vs returning */
  s.prop17=s.eVar16=s.getNewRepeat(365);
  
  /* Login status */
  if(s.prop18&&!s.eVar17) s.eVar17="D=c18";
  
    /* Set internal search term variables */
  if(s.prop14){
    s.prop14=s.prop14.toLowerCase();
  	s.eVar14="D=c14";
	if(s.prop16&&!s.eVar15) s.eVar15="D=c16";
  
  /* Set de-duped onsite search event */
  let t_search=s.getValOnce(s.eVar14,'ev14_STO',0);
  	if(t_search){
  		s.events=s.apl(s.events,'event1',',',1);
      if(!s.products) s.products=';';

  /* Set null-searches onsite search event */
  ...
  if(s.events.indexOf('event32')>-1)
	s.eVar21='stop';
  s.eVar21=s.getTimeToComplete(s.eVar21,'ttcc',0);

  /* Provide code version change notes detail number */   
  if(sc_code_ver){s.prop41=sc_code_ver;} else {s.prop41="unknown";}  

  /* Provide additional details based on qs values */  
  s.prop42=s.getQueryParam('vgnextoid');
  s.prop43=s.getQueryParam('page');
  s.prop44=s.getQueryParam('formtype');
  s.prop45=s.getQueryParam('let');
...
/*                                                                  
* Plugin: clickPast + p_fo - version 1.0
*/
...
s.visitorNamespace="ferroviedellostato"
s.trackingServer="ferroviedellostato.d3.sc.omtrdc.net"
...
let s_code='',s_objectID;function s_gi(un,pg,ss){let c="s.version='H.24.1';s.an=s_an;s.logDebug=function(m){var s=this,tcf=new Function('var e;try{console.log(\"'+s.rep(s.rep(m,\"\\n\",\"\\\\n\"),\""
...
function s_giqf(){let w=window,q=w.s_giq,i,t,s;if(q)for(i=0;i<q.length;i++){t=q[i];s=s_gi(t.oun);s.sa(t.un);s.setTagContainer(t.tagContainerName)}w.s_giq=0}s_giqf()