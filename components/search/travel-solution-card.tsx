...
export function TravelSolutionCard({
  ...
  return (
    <Pressable
      ...
      className="rounded-lg bg-white p-5 border border-gray-100"
    >
      {/* Top Row: Logos & Price */}
      ...
      {solution.price > 0 &&
        solution.trains.some(
          ...
          <View className="mt-5 flex-row items-center gap-3 rounded-lg bg-gray-50 p-4 border border-gray-100">
      ...
      {solution.price > 0 &&
        solution.trains
          ...
          .map((regTrain, idx) => (
            <View
              key={idx}
              className="mt-5 flex-row items-center gap-3 rounded-lg bg-gray-50 p-4 border border-gray-100"
            >
              <Icon name="train" size={20} className="!text-gray-800" />
              <ThemedText className="flex-1 text-[13px] font-plus-jakarta-medium !text-gray-800 !leading-tight">
                [{regTrain.type.toLowerCase().includes("regv") ? "Regionale Veloce" : "Regionale"} {regTrain.number}] {getAvailableSeats(regTrain.number)} posti acquistabili
              </ThemedText>
            </View>
          ))}
    </Pressable>
