...
export function CalendarPanel({
	...
	const [currentMonth, setCurrentMonth] = useState(
		initialTab === "andata" ? departureDate : returnDate,
	);
			...
			>
				<View className="flex-1 justify-between flex-col pb-2">
					<View className="-mx-6 px-[4px]">
						...
						{/* Days Grid */}
						<View className="px-0 flex-row flex-wrap h-[280px]">
								...
								>
									{d.day && (
										<>
											...
											{d.isInRange && (
												<View className="absolute inset-y-1 left-0 right-0 bg-primary-500/10" />
											)}
											{d.isStart && !d.isEnd && (
												<View className="absolute inset-y-1 left-1/2 right-0 bg-primary-500/10" />
											)}
											{d.isEnd && !d.isStart && (
												<View className="absolute inset-y-1 left-0 right-1/2 bg-primary-500/10" />
											)}
											...
											<View
												className={`w-10 h-10 items-center justify-center relative z-10 ${
													d.isSelected
														...
														: d.isStart || d.isEnd
															? "rounded-full border-2 border-primary-500 bg-white"
															: ""
												} ${d.isPast ? "opacity-50" : ""}`}
											>
												<ThemedText
