...
import { Image, Pressable, ScrollView, View, ActivityIndicator, Alert } from "react-native";
...
import { TimelineStation, TimelineEvent } from "@/constants/train-details-mock";
...
function formatStationName(rawName: string) {
	const found = STATIONS.find(s => s.name.toLowerCase() === rawName.toLowerCase());
...
export default function TrainDetailsScreen() {
	...
	useEffect(() => {
		if (!trainNumber) {
			Alert.alert("Errore", "Nessun numero di treno specificato", [{ text: "OK", onPress: () => router.back() }]);
			return;
		}
		...
		async function fetchTrainDetails() {
			try {
				...
				if (!autoText || autoText.trim() === "") {
					Alert.alert("Treno non trovato", "Il treno inserito non è stato trovato o non circola oggi.", [{ text: "OK", onPress: () => router.back() }]);
					return;
				}
				...
				const mappedStations: TimelineStation[] = data.fermate.map(f => {
					...
					if (f.arrivo_teorico !== null || f.arrivoReale !== null || f.tipoFermata === "A") {
						events.push({ label: "Arrivo Programmato", time: formatTime(f.arrivo_teorico || f.programmata) });
						if (f.arrivoReale || f.effettiva) {
							events.push({ label: "Arrivo Effettivo", time: formatTime(f.arrivoReale || f.effettiva), isActual: true });
						}
					}

					// Departure
					if (f.partenza_teorica !== null || f.partenzaReale !== null || f.tipoFermata === "P") {
						events.push({ label: "Partenza Programmata", time: formatTime(f.partenza_teorica || f.programmata) });
						if (f.partenzaReale || f.effettiva) {
							events.push({ label: "Partenza Effettiva", time: formatTime(f.partenzaReale || f.effettiva), isActual: true });
						}
					}
				...
				Alert.alert("Errore", `Dettaglio: ${err.message}`, [{ text: "OK", onPress: () => router.back() }]);
			} finally {
				setLoading(false);
			}
		}
	...
	let delayText = trainData.compRitardo && trainData.compRitardo.length > 0 ? trainData.compRitardo[0] : "";
	...
	const delayBgColor = isDelay ? 'bg-red-50 border-red-200' : 'bg-[#005045]/10 border-[#005045]/20';
	const delayTextColor = isDelay ? '!text-red-500' : '!text-[#005045]';

	const lastDetection = trainData.stazioneUltimoRilevamento !== "--" ? formatStationName(trainData.stazioneUltimoRilevamento) : "Nessun rilevamento";
	const lastDetectionTime = trainData.compOraUltimoRilevamento !== "--" ? ` - ${trainData.compOraUltimoRilevamento}` : "";
				...
				<View className="border border-gray-200 rounded-lg p-4 mb-8">
					<View className="flex-row items-center justify-between">
						<View className="flex-1 mr-4">
							<ThemedText className={`font-plus-jakarta-bold !text-gray-950 ${lastDetection !== "Nessun rilevamento" ? 'mb-1' : ''}`}>
								{lastDetection}
							</ThemedText>
							{lastDetection !== "Nessun rilevamento" && (
								<ThemedText className="text-xs font-plus-jakarta-medium !text-gray-500">
									Ultimo rilevamento: {formattedDate}{lastDetectionTime}
								</ThemedText>
							)}
						</View>
						<View className="flex-row items-center">
							<View className={`px-2 py-1 rounded border ${delayBgColor}`}>
								<ThemedText className={`text-sm font-plus-jakarta-bold ${delayTextColor}`}>
									{delayText}
								</ThemedText>
							</View>
						</View>
