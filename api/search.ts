...
async function getLocationId(stationName: string): Promise<number> {
	...
	let url = getLefrecceUrl(
		`/locations/search?name=${encodeURIComponent(stationName)}&limit=1`,
	);
	...
	try {
		...
		const routes: TrenitRoute[] = filteredSolutions.map((item: any) => {
			...
			const legs: TrenitLeg[] = (sol.nodes || []).map((node: any) => {
				...
				if (
					node.train.logoId === "TTPER" ||
					(node.train.denomination && node.train.denomination.includes("TTPER"))
				) {
			...
			const isSaleable =
				sol.status === "SALEABLE" &&
				item.grids &&
				totalCoveredNodes === (sol.nodes || []).length;
			route.saleable = isSaleable;

			if (
				isSaleable &&
				sol.price &&
				sol.price.amount !== undefined &&
				sol.price.amount !== null
			) {
			...
			if (isSaleable && item.grids && item.grids.length > 0) {
				...
				if (firstGrid.services && firstGrid.services.length > 0) {
					const service =
						firstGrid.services.find(
							(s: any) => s.id === firstGrid.selectedServiceId,
						) || firstGrid.services[0];
					...
					if (service && service.offers && service.offers.length > 0) {
						const offer =
							service.offers.find(
								(o: any) => o.offerId === firstGrid.selectedOfferId,
							) || service.offers[0];
				...
				route.tk = item.grids.map((grid: any, gridIdx: number) => {
					...
					(grid.services || []).forEach((service: any) => {
						if (service.name)
							classes.push(service.name.replace(/ PRENOTAZIONE/i, "").trim());
				...
				let fallbackOffer =
					sol.trains && sol.trains.length > 0
						? sol.trains[0].trainCategory
						: "Standard";
				route.tk = [
					{
						i: [0],
