...
`;
content = content.replace('const CHIPS = ["N. Treno", "Tabellone", "Da/a", "Treni seguiti"];', 'const CHIPS = ["Stazione", "Da/a", "N. Treno"];\n' + animatedTabLabelCode);
...
`;
content = content.replace('const [recentTrains, setRecentTrains] = useState<RecentTrainSearch[]>([]);\n', 'const [recentTrains, setRecentTrains] = useState<RecentTrainSearch[]>([]);\n' + animatedStyleCode + '\n');

// 4. Update switch statement cases
content = content.replace('case "Tabellone":', 'case "Stazione":');

// 5. Replace top chips with the new animated selector safely
const targetHeader = `					<ScrollView
						horizontal
					...
					>
						{CHIPS.map((chip) => (
							...
							>
								<ThemedText className="font-google-sans-semibold !text-white">
									{chip}
								</ThemedText>
							</Pressable>
						))}
...
content = content.replace(targetHeader, tabSelectorCode);

// Update fixed banner for Tabellone -> Stazione
content = content.replace('activeChip === "Tabellone"', 'activeChip === "Stazione"');
